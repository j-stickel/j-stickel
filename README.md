...
**AI Systems Engineer** - I design, ship, and operate production AI systems end to end: RAG pipelines, agentic assistants, fine-tuned models, and the platforms to run them. Solo, in production, measured. Retrieval recall on my flagship system: driven from 40% to a measured 98% floor on a frozen regression set - and the LLM judge that grades every answer is itself calibrated five ways (planted errors 29/29, human-adjudicated kappa 0.61, second-lab agreement 92.7%, an independently-authored second harness - RAGAS - agreeing 98.2% at the pass/fail boundary, and a locked holdout exam written by an outside model: my measured overfit to my own test is 6.3 points, published). I do not just publish numbers; I grade the grader behind them.
...
### Now (updated 2026-07-25)

- **Grading the grader:** the LLM judge behind every trust number is now calibrated five independent ways - planted-error suite 29/29 across four rubrics, human-adjudicated agreement 90% (kappa 0.61), a second judge from a different lab at 92.7% agreement with all 8 disagreements hand-adjudicated, and a second independently-authored harness (RAGAS) re-grading the same stored run with the same judge model: 98.2% agreement at the pass/fail boundary, its sole disagreement an already-known defect row. Re-grading after an instrument fix moved exactly one verdict - the one it targeted (kappa 0.930, zero collateral).
- **The exam nobody could study for:** the fifth instrument is a locked holdout - 18 questions authored by an outside model straight from the corpus, structurally excluded from every fix loop. First run: tuned questions 89.6%, the holdout 83.3% - a 6.3-point overfit gap published as a first-class number, recomputed on every future run. Retrieval went 18 for 18 on the stranger's questions - twice now - pinning the whole gap to the answer layer.
- **Refuse-vs-fabricate, measured:** twelve questions that demand artifacts the corpus does not hold (raw CSVs, shell commands, latency percentiles) now run in every eval, scored by a fourth judge rubric - any invented path, command, or figure fails, even inside a disclosure-toned answer. First run: 12/12, with the prediction registered in advance at 8-11 and the planted suite arbitrating the difference: all four planted fabrication modes caught on the first pass. The honesty behavior an external stress test praised, converted into a number that recomputes every run.
