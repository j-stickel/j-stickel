...
- **Governed agentic writes (new):** a demo agent that *changes* records, not just answers - Claude tool use against a mock of Workday's documented Staffing API, with authorization checked against session identity (never against anything the model says), a server-side confirm-before-execute gate, idempotent writes, and refusals audited as faithfully as actions. Measured with a mechanically-graded eval suite - 18/18 on the latest run, prompt-injection cases 5/5 - and the trust layer's own tests pass with no model attached. [Two-minute demo](https://mrjstickel.com/projects/agentic-hr-demo).
